...
<a id = "Basic"></a>
## 基本事項
- サーブレットとは，Javaのサーバ上で動作するweb アプリケーションを実現するためのプログラムのこと(CGIみたいなもの)．
...
1. リンクを移動するだけならhtml のaタグだけで済むが、静的ページ以外のページを要求してきたり、情報を入力してそれを登録するようなアクションがあった場合、フォームのaction タグに書かれたURL アノテーションを持つようなコントローラクラスに処理が遷移する。アノテーションを使わない場合には、web.xml がこれを担当する。
1. コントローラクラスに処理が飛んだら、さらにURL アノテーションと一致するGET かPOST 関数に処理が飛ぶ。そこで何らかの処理を行ってから最後にDispatch クラスのforward() で、処理をユーザへの応答を担当しているJSP のページに飛ばす。
...
理由としては、Servlet/JSP による開発はMVC モデルが目指していた完全なる分業とは程遠いものだという点と、バリデーションチェックやログイン機構などのweb アプリを作成する上は必須の機能が予め用意されておらず、自分で一から実装しなければならない点にある(車輪の再開発にも程がある)。
